...
public:
	...
	void export_file(generate maze) {
		...
		cin >> export_name;
		export_name += ".txt";
		// Datei wird bei gleichem namen �bershcrieben
	...
	void import_file(generate* maze) {
		// Wir holen uns den aktuellen Pfad, von wo unser Program aufgerufen wurde. In diesem Dircectory/Ordner m�ssen auch unsere txt Dateien liegen.
		path = std::filesystem::current_path().string();
		vector<std::filesystem::directory_entry> files_found;
		int file_choice;
		string current_file_path;
		// loopt �ber alle Datein in einem gegbenen ordner (nur in CPP17 verf�gbar)
		...
		// fr�her early return um sicherzustellen das ab jetzt nur noch mit der Bedingung gearbietet werden kann, dass es Spielfelder gibt.
		if (files_found.empty())
		{
			cout << "Keine Spielfelder Gefunden\n";
			return;
		}
		cout << "w�hlen Sie eine Datei aus:\n";
		...
		// Maze muss einem neuen Speicherbereich zugewiesen werden, da sich die spiechergr�sse, im Vergleich zum Konstruktor ver�ndert hat.
		maze->maze = /* die eingabe f�r maze wird in ein int* gewandelt(gecasted) */(int*)calloc(maze->row * maze->col, sizeof(int));

		// schnelle L�sung einen Char zu einem Int zu machen.
		string str2char = "";
		...
		// Da wir davon ausgehen, dass der Index hier bei 1 beginnt also unsere erste Startposition (1,1) w�re, m�ssen wir -1 rechnen um es an ein Array anzupassen.
...
private:
	string export_name;
	string import_name;
	string path = ""; // Hier k�nnte Backup Pfad stehen ...
